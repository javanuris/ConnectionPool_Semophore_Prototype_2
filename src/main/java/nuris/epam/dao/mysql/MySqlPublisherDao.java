...
public class MySqlPublisherDao extends PublisherDao {
    ...
    private static final String FIND_BY_BOOK = Sql.create().select().varS(PUBLISHER, ID_PUBLISHER).c().varS(PUBLISHER, NAME).c().varS(PUBLISHER, CITY).from().var(PUBLISHER).join(BOOK).varS(BOOK, ID_PUBLISHER).eq().varS(PUBLISHER, ID_PUBLISHER).whereQs(BOOK, ID_BOOK).build();
